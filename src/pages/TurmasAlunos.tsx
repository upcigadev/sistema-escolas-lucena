  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="rounded-xl border bg-card shadow-card overflow-hidden hidden md:block"
      >
                ...
                <div className="border-t border-border bg-muted/30 p-3">
                  <div className="grid grid-cols-1 gap-2">
                    {students.map((s) => (
                      ...
                      >
                        <User className={`h-3.5 w-3.5 shrink-0 ${s.present ? "text-success" : "text-destructive"}`} />
                        <span className="truncate text-foreground font-medium text-xs">{s.name}</span>
                        {s.arrivalTime && (
                          <span className="ml-auto flex items-center gap-0.5 text-[10px] text-muted-foreground">
                            <Clock className="h-2.5 w-2.5" />
                            {s.arrivalTime}
